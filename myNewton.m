...
if ~exist('dfunc','var')
    diffmethod = questdlg('Choose a differentiation method:', 'Numerical Differentiation', 'forward', 'backward', 'central', 'forward');
    dfunc = @(x) numDiff(diffmethod, func, x);
    disp(['Using default derivative: dfunc = numDiff with method ', diffmethod]);
end
